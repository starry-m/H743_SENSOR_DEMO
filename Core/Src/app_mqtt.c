...
static void bsp_mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    if( (data == NULL) || (len == 0) )
    {
        LOG_I("mqtt_client_incoming_data_cb: condition error @entry\n");
        return;
    }
    ...
    LOG_I("mqtt_client_incoming_data_cb:reveiving incomming data.\n");
}
...
static void bsp_mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
    ...
    {
        LOG_I("bsp_mqtt_incoming_publish_cb: condition error @entry\n");
        return;
    }

	LOG_I("bsp_mqtt_incoming_publish_cb: topic = %s.\n",topic);
	LOG_I("bsp_mqtt_incoming_publish_cb: tot_len = %d.\n",tot_len);
...
static void bsp_mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    if( client == NULL )
    {
        LOG_I("bsp_mqtt_connection_cb: condition error @entry\n");
        return;
    }

    if ( status == MQTT_CONNECT_ACCEPTED ) //Successfully connected
    {
		LOG_I("bsp_mqtt_connection_cb: Successfully connected\n");
	...
	{
		LOG_I("bsp_mqtt_connection_cb: Fail connected, status = %s\n", lwip_strerr(status) );
        //错误处理
...
{
    LOG_I("bsp_mqtt_connect: Enter!\n");
	err_t ret;

    struct mqtt_connect_client_info_t  mqtt_connect_info = {
		"STM32_MQTTX_Test",  /* 这里需要修改，以免在同一个服务器两个相同ID会发生冲突 */
		"123",   /* MQTT 服务器用户名 */
		"321",   /* MQTT 服务器密码 */
    ...
    ip_addr_t server_ip;
    ip4_addr_set_u32(&server_ip, ipaddr_addr("123.60.157.221"));  //MQTT服务器IP?  123.60.157.221
//    IP4_ADDR(&server_ip, 123, 60, 157, 221); //这里填自己云平台的ip地址
	...
	{
        //防止申请失败
		LOG_I("bsp_mqtt_connect: s__mqtt_client_instance malloc fail @@!!!\n");
		return ERR_MEM;
	}
    ...
    *****************/
    ...
    LOG_I("bsp_mqtt_connect: connect to mqtt %s\n", lwip_strerr(ret));
...
static void mqtt_client_pub_request_cb(void *arg, err_t result)
{
    ...
    if (result != ERR_OK)
    {
        LOG_I("mqtt_client_pub_request_cb: c002: Publish FAIL, result = %s\n", lwip_strerr(result));
	...
	{
        LOG_I("mqtt_client_pub_request_cb: c005: Publish complete!\n");
	}
}
...
err_t bsp_mqtt_publish(mqtt_client_t *client, char *pub_topic, char *pub_buf, uint16_t data_len, uint8_t qos, uint8_t retain)
{
	...
	{
		LOG_I("bsp_mqtt_publish: input error@@" );
		return ERR_VAL;
	}
    ...
    {
		LOG_I("bsp_mqtt_publish: client is not connected\n");
        return ERR_CONN;
    }
    ...
    {
		LOG_I("bsp_mqtt_publish: create mqtt mutex ! \n" );
...

    {
	    ...
	    LOG_I("bsp_mqtt_publish: mqtt_publish err = %s\n", lwip_strerr(err) );

#ifdef USE_MQTT_MUTEX
        LOG_I("bsp_mqtt_publish: mqtt_publish xSemaphoreTake\n");
...
static void bsp_mqtt_request_cb(void *arg, err_t err)
{
    if ( arg == NULL )
    {
        LOG_I("bsp_mqtt_request_cb: input error@@\n");
        return;
    }
    ...
    if ( err != ERR_OK )
    {
        LOG_I("bsp_mqtt_request_cb: FAIL sub, sub again, err = %s\n", lwip_strerr(err));
	...
	{
		LOG_I("bsp_mqtt_request_cb: sub SUCCESS!\n");
	}
}
...
static err_t bsp_mqtt_subscribe(mqtt_client_t* mqtt_client, char * sub_topic, uint8_t qos)
{
    LOG_I("bsp_mqtt_subscribe: Enter\n");

	if( ( mqtt_client == NULL) || ( sub_topic == NULL) || ( qos > 2 ) )
	{
        LOG_I("bsp_mqtt_subscribe: input error@@\n");
		return ERR_VAL;
	}

	if ( mqtt_client_is_connected(mqtt_client) != pdTRUE )
	{
		LOG_I("bsp_mqtt_subscribe: mqtt is not connected, return ERR_CLSD.\n");
		return ERR_CLSD;
	}
	...
	if (err != ERR_OK)
	{
		LOG_I("bsp_mqtt_subscribe: mqtt_subscribe Fail, return:%s \n", lwip_strerr(err));
	}
	else
	{
		LOG_I("bsp_mqtt_subscribe: mqtt_subscribe SUCCESS, reason: %s\n", lwip_strerr(err));
	}
