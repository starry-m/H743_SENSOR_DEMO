...
#include "usart.h"
    ...
    void MX_MEMS_Process(void)
    {
        ...
        {
            //    	printf("result:%d\r\n", state_back);
            snprintf(send_buff, 20, "result:%d\r\n", state_back);
            HAL_UART_Transmit(&huart3, send_buff, 20, 1000);
        }
        //    HAL_Delay(5);
    ...
    void MX_IKS4A1_DataLogTerminal_Init(void)
    {
        ...
        int i;
        MX_UART_InitTypeDef m_com = {.BaudRate = 115200,
        ...
        for (i = 0; i < IKS4A1_MOTION_INSTANCES_NBR; i++)
        {
            ...
            snprintf(dataOut, MAX_BUF_SIZE, " MAX GYRO ODR: %d.%03d Hz, MAX GYRO FS: %d\r\n",
                     (int)out_value_odr.out_int, (int)out_value_odr.out_dec, (int)MotionCapabilities[i].GyroMaxFS);
            printf("%s", dataOut);
    ...
    static void Gyro_Sensor_Handler(uint32_t Instance)
    {
        ...
        else
        {
            snprintf(dataOut, MAX_BUF_SIZE, "\r\nGYR_X[%d]: %d, GYR_Y[%d]: %d, GYR_Z[%d]: %d\r\n", (int)Instance,
                     (int)angular_velocity.x, (int)Instance, (int)angular_velocity.y, (int)Instance,
